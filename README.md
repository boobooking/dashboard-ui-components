...
Пакет экспортирует тринадцать компонентов: `Popup`, `Dot`, `PickDay`,
`RussianMobileFilter`, `Search`, `SelectDateInterval`, `SelectSingle`,
`SmallBadge`, `ErrorMessages`, `Closer`, `DownloadLink`, `ConfirmationModal`,
`DropdownButtonWithAction`. В архив пакета (`files: ["dist"]`) исходники не
...
Поднимает демо-страницы со всеми тринадцатью компонентами в двух окружениях:
...
внутри `Popup` и `DropdownButtonWithAction`. В приложении с preflight это
ничего не меняет; без него содержимое слота получит `box-sizing` и снятые
границы.
